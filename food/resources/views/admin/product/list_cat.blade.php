...
<div id="content" class="container-fluid">
    <div class="row">
        ...
        <div class="col-8">
            <div class="card">
                <div class="card-header font-weight-bold">
                    Danh sách ({{ count($result1) }} danh mục)
                </div>
                <div class="card-body">
                        ...
                        <tbody>
                            ...
                            @foreach ($result1 as $item)
                                ...
                                <tr>
                                    ...
                                    <td style="{{ $item->parent_id == 0?'font-weight:bold':'' }}">{{ $item->name }}</td>
                                    <td>{{ $item->slug }}</td>
                                    <td>{{ $item->status->name }}</td>
                                    <td>
                                        <a href="{{ route('product/cat/edit',$item->id) }}" class="btn btn-success btn-sm rounded-0 text-white" type="button" data-toggle="tooltip" data-placement="top" title="Edit"><i class="fa fa-edit"></i></a>
                                        <a href="{{ route('product/cat/delete',$item->id) }}" onclick="return confirm('Bạn có muốn xoá danh muc ?')" class="btn btn-danger btn-sm rounded-0 text-white" type="button" data-toggle="tooltip" data-placement="top" title="Delete"><i class="fa fa-trash"></i></a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
